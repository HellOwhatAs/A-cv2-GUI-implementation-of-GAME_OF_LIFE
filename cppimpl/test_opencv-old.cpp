...
using namespace std;
using namespace cv;
int l = 200, windowl = 640, sight = windowl, pressing = 0, fps = 50;
bool mode = 0;
...
pair<pair<int, int>, pair<int, int>> get_cutslice(pair<int, int> cut) {
	return {
		{round(sight_point.first - cut.first * double(sight) / l),round(sight_point.first - cut.first * double(sight) / l) + windowl},
		{round(sight_point.second - cut.second * double(sight) / l),round(sight_point.second - cut.second * double(sight) / l) + windowl}
	};
}
